...
LitColorTextureProgram::LitColorTextureProgram() {
	...
	,
		...
		"	}\n"
		"	float nl = dot(n,l);\n"
		" 	vec3 color = (color.rgb / 3.1415926) * LIGHT_ENERGY * (smoothstep(0.0,0.1,nl) * 0.6 + 0.4) * (smoothstep(-1.0,0.1,nl) * 0.6 + 0.4) ;\n"
		"	vec4 albedo = texture(TEX, texCoord) * vec4(color, 1.0);\n"
